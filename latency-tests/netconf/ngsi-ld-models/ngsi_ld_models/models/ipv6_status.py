...
class Ipv6Status(BaseModel):
    """
    NGSI-LD Property Type. The status of an address. Most of the states correspond to states from the IPv6 Stateless Address Autoconfiguration protocol.   # noqa: E501
    """
    ...
    def value_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('preferred', 'deprecated', 'invalid', 'inaccesible', 'unknown', 'tentative', 'duplicate', 'optimistic'):
            raise ValueError("must be one of enum values ('preferred', 'deprecated', 'invalid', 'inaccesible', 'unknown', 'tentative', 'duplicate', 'optimistic')")
        return value
    ...
    def from_dict(cls, obj: dict) -> Self:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in Ipv6Status) in the input: " + _key)
