...
# from ngsi_ld_models.models.address import Address
interface_ipv4_address_dict_buffers = []

# from ngsi_ld_models.models.neighbor import Neighbor
...
# from ngsi_ld_models.models.autoconf import Autoconf
...
for interface in root.findall(".//{urn:ietf:params:xml:ns:yang:ietf-interfaces}interface"):
    ...
    for ipv4 in interface.findall(".//{urn:ietf:params:xml:ns:yang:ietf-ip}ipv4"):
        ...
        if enabled is not None:
            ...
            interface_ipv4_dict_buffer["enabled"]["value"] = element_text.capitalize()
        ...
        if forwarding is not None:
            ...
            interface_ipv4_dict_buffer["forwarding"]["value"] = element_text.capitalize()
        ...
        for address in ipv4.findall(".//{urn:ietf:params:xml:ns:yang:ietf-ip}address"):
            ...
            interface_ipv4_address_dict_buffer["id"] = "urn:ngsi-ld:Address:" + interface_ipv4_dict_buffer["id"].split(":")[-1]
            interface_ipv4_address_dict_buffer["type"] = "Address"
        ...
        for neighbor in ipv4.findall(".//{urn:ietf:params:xml:ns:yang:ietf-ip}neighbor"):
            ...
            interface_ipv4_neighbor_dict_buffer["type"] = "Neighbor"
    ...
    for ipv6 in interface.findall(".//{urn:ietf:params:xml:ns:yang:ietf-ip}ipv6"):
        ...
        if enabled is not None:
            ...
            interface_ipv6_dict_buffer["enabled"]["value"] = element_text.capitalize()
        ...
        if forwarding is not None:
            ...
            interface_ipv6_dict_buffer["forwarding"]["value"] = element_text.capitalize()
        ...
        for address in ipv6.findall(".//{urn:ietf:params:xml:ns:yang:ietf-ip}address"):
            ...
            interface_ipv6_address_dict_buffer["id"] = "urn:ngsi-ld:Address:" + interface_ipv6_dict_buffer["id"].split(":")[-1]
            interface_ipv6_address_dict_buffer["type"] = "Address"
        ...
        for neighbor in ipv6.findall(".//{urn:ietf:params:xml:ns:yang:ietf-ip}neighbor"):
            ...
            interface_ipv6_neighbor_dict_buffer["type"] = "Neighbor"
        ...
        for autoconf in ipv6.findall(".//{urn:ietf:params:xml:ns:yang:ietf-ip}autoconf"):
            ...
            interface_ipv6_autoconf_dict_buffer["id"] = "urn:ngsi-ld:Autoconf:" + interface_ipv6_dict_buffer["id"].split(":")[-1]
            interface_ipv6_autoconf_dict_buffer["type"] = "Autoconf"
            ...
            if createGlobalAddresses is not None:
                ...
                interface_ipv6_autoconf_dict_buffer["createGlobalAddresses"]["value"] = element_text.capitalize()
            ...
            if createTemporaryAddresses is not None:
                ...
                interface_ipv6_autoconf_dict_buffer["createTemporaryAddresses"]["value"] = element_text.capitalize()
...
# Manual modification to send dictionary buffers to a Kafka topic.
# Topic name is dictionary_buffers - it must be created previously.
...
for interface_dict_buffer in interface_dict_buffers:
    producer.poll(0)
    producer.produce('dictionary_buffers', str(interface_dict_buffer).encode('utf-8'), callback=delivery_report)
producer.flush()

for interface_statistics_dict_buffer in interface_statistics_dict_buffers:
    producer.poll(0)
    producer.produce('dictionary_buffers', str(interface_statistics_dict_buffer).encode('utf-8'), callback=delivery_report)
producer.flush()

for interface_ipv4_dict_buffer in interface_ipv4_dict_buffers:
    producer.poll(0)
    producer.produce('dictionary_buffers', str(interface_ipv4_dict_buffer).encode('utf-8'), callback=delivery_report)
producer.flush()

for interface_ipv4_address_dict_buffer in interface_ipv4_address_dict_buffers:
    producer.poll(0)
    producer.produce('dictionary_buffers', str(interface_ipv4_address_dict_buffer).encode('utf-8'), callback=delivery_report)
producer.flush()

for interface_ipv4_neighbor_dict_buffer in interface_ipv4_neighbor_dict_buffers:
    producer.poll(0)
    producer.produce('dictionary_buffers', str(interface_ipv4_neighbor_dict_buffer).encode('utf-8'), callback=delivery_report)
producer.flush()

for interface_ipv6_dict_buffer in interface_ipv6_dict_buffers:
    producer.poll(0)
    producer.produce('dictionary_buffers', str(interface_ipv6_dict_buffer).encode('utf-8'), callback=delivery_report)
producer.flush()

for interface_ipv6_address_dict_buffer in interface_ipv6_address_dict_buffers:
    producer.poll(0)
    producer.produce('dictionary_buffers', str(interface_ipv6_address_dict_buffer).encode('utf-8'), callback=delivery_report)
producer.flush()

for interface_ipv6_neighbor_dict_buffer in interface_ipv6_neighbor_dict_buffers:
    producer.poll(0)
    producer.produce('dictionary_buffers', str(interface_ipv6_neighbor_dict_buffer).encode('utf-8'), callback=delivery_report)
producer.flush()

for interface_ipv6_autoconf_dict_buffer in interface_ipv6_autoconf_dict_buffers:
    producer.poll(0)
    producer.produce('dictionary_buffers', str(interface_ipv6_autoconf_dict_buffer).encode('utf-8'), callback=delivery_report)
producer.flush()