...
def generate_python_json_parser_code(ctx, modules, fd):
    ...
    if (ctx.opts.candil_json_parser_generator_kafka_server is not None) and \
        ...
        READING_INSTRUCTIONS_KAFKA = [
            'dict_buffers = []\n',
            'consumer = KafkaConsumer(\'' + ctx.opts.candil_json_parser_generator_kafka_input_topic + '\', bootstrap_servers=[\'' + ctx.opts.candil_json_parser_generator_kafka_server + '\'], value_deserializer=lambda x: json.loads(x.decode(\'utf-8\')))\n',
            'while True:\n',
    ...
    def generate_parser_code(element, parent_element_arg, entity_path: str, camelcase_entity_path: str, depth_level: int, typedefs_dict: dict, transition_element, modules_name: list, yang_data_nodes_list: list):
        ...
        ### NGSI-LD PROPERTY IDENTIFICATION ###
            ...
            if (ctx.opts.candil_json_parser_generator_source != "netflow") and ('name'.casefold() in str(element.arg) or ('id'.casefold() in str(element.arg).casefold())):
            ...
            if (ctx.opts.candil_json_parser_generator_source == "netflow"):
                if ('number'.casefold() in str(element.arg).casefold()) or ('flow-id'.casefold() == str(element.arg).casefold()) or ('name'.casefold() in str(element.arg)) or ('sampler-address'.casefold() in str(element.arg)): 
                    text_format = str(text_format)
                    fd.write('\n' + INDENTATION_BLOCK * depth_level + INDENTATION_BLOCK + 'if ' + current_path.replace(str(element.arg) + '_', '').replace('-', '_') + 'dict_buffer[\"id\"].split(\":\")[-1]' + ' != ' + text_format + ":")  
                    fd.write('\n' + INDENTATION_BLOCK * depth_level + INDENTATION_BLOCK + INDENTATION_BLOCK + current_path.replace(str(element.arg) + '_', '').replace('-', '_') + 'dict_buffer[\"id\"] = ' + current_path.replace(str(element.arg) + '_', '').replace('-', '_') + 'dict_buffer[\"id\"] + ":" + str(element_text)')
                elif ('source-id'.casefold() == str(element.arg).casefold()):
                    text_format = str(text_format)
                    fd.write('\n' + INDENTATION_BLOCK * depth_level + INDENTATION_BLOCK + 'if ' + current_path.replace(str(element.arg) + '_', '').replace('-', '_') + 'dict_buffer[\"id\"].split(\":\")[3]' + ' != ' + text_format + ":")  
                    fd.write('\n' + INDENTATION_BLOCK * depth_level + INDENTATION_BLOCK + INDENTATION_BLOCK + current_path.replace(str(element.arg) + '_', '').replace('-', '_') + 'dict_buffer[\"id\"] = ":".join(' + current_path.replace(str(element.arg) + '_', '').replace('-', '_') + 'dict_buffer[\"id\"].split(\":\")[:3]) + ":" + str(element_text) + ":" + ":".join(' + current_path.replace(str(element.arg) + '_', '').replace('-', '_') + 'dict_buffer[\"id\"].split(\":\")[3:])' )
        ...
        ### NGSI-LD RELATIONSHIP IDENTIFICATION ###
            ...
            if (ctx.opts.candil_json_parser_generator_source == "topology"):
                if ('source'.casefold() in str(element.arg)) or ('dest'.casefold() in str(element.arg)):
                    ...
                    fd.write('\n' + INDENTATION_BLOCK * depth_level + INDENTATION_BLOCK + entity_path.replace('-', '_') + 'dict_buffer[\"' + camelcase_element_arg + '\"][\"object\"] = \"urn:ngsi-ld:' + relationship_camelcase_path + ':\" + ' + entity_path.replace('-', '_') + 'dict_buffer[\"id\"].split(\":\")[3]' + " + ':' + \":\".join(" + entity_path.replace('-', '_') + 'dict_buffer[\"id\"].split(\":\")[5:])') 
            else:
                fd.write('\n' + INDENTATION_BLOCK * depth_level + INDENTATION_BLOCK + current_path.replace("_" + str(element.arg) + '_', '_', 1).replace('-', '_') + 'dict_buffer[\"' + camelcase_element_arg + '\"] = {}')
