...
def get_xpath_with_keys(xpath: str, entity_id: str, all_context_registries: Optional[list]) -> str:
    ...
    index = entity_id_split.index(entity_id_split[3])
...
'''
Build XPath with keys if the NGSI-LD entity ID is provided but with prefixes:
'''
def get_xpath_with_keys_with_prefixes(xpath: str, entity_id: str, all_context_registries: Optional[list]) -> str:
...
'''
Get XPath from Context Catalog by searching the context registry value (i.e., the entity type URI field) and including all namespace prefixes:
'''
def get_xpath_in_context_catalog_with_prefixes(entity_type: str, all_context_data: Optional[dict]) -> str:
    xpath = ""
    entity_type_short_founded = False
...
'''
Build XPath with '*' character included in the different keys if NGSI-LD entity ID is not provided:
'''
...
def generate_netconf_xml_config(xpath: str, all_context_data: Optional[dict], entity_type: str) -> str:
    ...
    search_original_context_data = entity_type #all_context_data[entity_type]
...
def get_operation(host: str, port: str, username: str, password: str, family: str, entity_type: str, entity_id: str, option: str, all_context_data: Optional[dict] = None, hostKeyVerify: Optional[bool] = False, sysAttrs: Optional[bool] = False, all_context_registries: Optional[list] = None):
    ...
    producer.send('interfaces-state-subscriptions', value=query_xml)
