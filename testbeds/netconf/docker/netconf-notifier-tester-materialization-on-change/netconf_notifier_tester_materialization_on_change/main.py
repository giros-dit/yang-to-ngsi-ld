...
from netconf_notifier_tester_materialization_on_change.check_client import NGSILDHealthInfoClient
...
from ngsi_ld_models_mdt_client_data_materialization.models.name import Name
from ngsi_ld_models_mdt_client_data_materialization.models.address import Address
from ngsi_ld_models_mdt_client_data_materialization.models.port import Port
from ngsi_ld_models_mdt_client_data_materialization.models.username import Username
from ngsi_ld_models_mdt_client_data_materialization.models.password import Password
...
from ngsi_ld_models_mdt_client_data_materialization.models.period import Period
...
# Notifier
NOTIFIER_URI = os.getenv("NOTIFIER_URI", "http://netconf-notifier-tester-materialization-on-change:8082/notify")
...
performance_measurements_file = open("/opt/netconf-notifier-tester-materialization-on-change/netconf_notifier_tester_materialization_on_change/performance_measurements.csv", "w", newline='')
...
notification_performance_measurements_file = open("/opt/netconf-notifier-tester-materialization-on-change/netconf_notifier_tester_materialization_on_change/notification_performance_measurements.csv", "w", newline='')
...
async def startup_event():
    ...
    subs_mode_instance = SubscriptionMode(
        type="Property",
        value="periodic",
        period=Period.from_dict({"type": "Property", "value": 500})
    )

    operation_instance = Operation(
        type="Property",
        value="SUBSCRIBE",
        entityType={"type": "Property", "value": "ietf-interfaces:interfaces-state/interface/statistics"},
        entityId={"type": "Property", "value": "urn:ngsi-ld:InterfaceStatistics:clab-telemetry-ixiac-lab:r1:GigabitEthernet2"},
        subscriptionMode=subs_mode_instance,
        subscriptionState={"type": "Property", "value": "ON"}
    )

    rpc_operation_instance = RpcOperation(
        type="RpcOperation",
        id="urn:ngsi-ld:RpcOperation:r1",
        operation=operation_instance,
        usesProtocol=UsesProtocol.from_dict({"type": "Relationship", "object": "urn:ngsi-ld:Protocol:r1"})
    )

    entity_input = rpc_operation_instance.to_dict()
    query_entity_input = QueryEntity200ResponseInner.from_dict(entity_input)
    try:
        # Create NGSI-LD entity of type RpcOperation: POST /entities
        api_instance.create_entity(query_entity200_response_inner=query_entity_input)
    except Exception as e:
        logger.exception("Exception when calling ContextInformationProvisionApi->create_entity: %s\n" % e)
    ...
    subs_request = CreateSubscriptionRequest (
        ...
        entities=[
            ...
            {
                "type": "ietf-interfaces:interfaces-state/interface"
            }
        ],
