...
from notifier_tester_virtualization_periodic_context_source.check_client import NGSILDHealthInfoClient
...
# Notifier
NOTIFIER_URI = os.getenv("NOTIFIER_URI", "http://notifier-tester-virtualization-periodic-context-source:8082/notify")
...
performance_measurements_file = open("/opt/notifier-tester-virtualization-periodic-context-source/notifier_tester_virtualization_periodic_context_source/performance_measurements.csv", "w", newline='')
...
notification_performance_measurements_file = open("/opt/notifier-tester-virtualization-periodic-context-source/notifier_tester_virtualization_periodic_context_source/notification_performance_measurements.csv", "w", newline='')
...
async def startup_event():
    ...
    for entity in LIST_ENTITIES:
        ...
        csources = api_instance_csource.query_csr(type=entity)
        print(csources)
        if csources != []:
            for csource in csources:
                ...
                subs_request = CreateSubscriptionRequest (
                    ...
                    entities=[
                        ...
                        {
                            "type": entity,
                            "id": "urn:ngsi-ld:InterfaceStatistics:clab-telemetry-ixiac-lab-r1:GigabitEthernet3"
                        }
                    ],
...
async def receiveNotification(request: Request):
    ...
    for entity in notification["data"]:
        if entity["type"] == "InterfaceStatistics" or entity["type"] == "Interface":
            ...
            if (entity["type"] == "InterfaceStatistics" and "observedAt" in entity["inOctets"] and "modifiedAt" in entity["inOctets"]) or (entity["type"] == "Interface" and "observedAt" in entity["enabled"] and "modifiedAt" in entity["enabled"]):
