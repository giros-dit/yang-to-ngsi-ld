...
#def batch_upsert_ngsi_ld_entities(ngsi_ld, query_entity_inputs) -> bool:
#def batch_upsert_ngsi_ld_entities(ngsi_ld, entities) -> bool:
    result = False
    
    api_instance = ngsi_ld_client.ContextInformationProvisionApi(ngsi_ld)

    #query_entity_inputs = [QueryEntity200ResponseInner.from_dict(e.to_dict()) for e in entities]
    query_entity_inputs = []
...
def get_entity_class_object_by_type(dict_buffer: dict):
    ...
    elif type == 'InterfaceIpv4':
        entity = InterfaceIpv4.from_dict(dict_buffer)
    elif type == 'InterfaceIpv4Address':
        entity = InterfaceIpv4Address.from_dict(dict_buffer)
    elif type == 'InterfaceIpv4Neighbor':
        entity = InterfaceIpv4Neighbor.from_dict(dict_buffer)
    elif type == 'InterfaceIpv6':
        entity = InterfaceIpv6.from_dict(dict_buffer)
    elif type == 'InterfaceIpv6Address':
        entity = InterfaceIpv6Address.from_dict(dict_buffer)
    elif type == 'InterfaceIpv6Autoconf':
        entity = InterfaceIpv6Autoconf.from_dict(dict_buffer)
    elif type == 'InterfaceIpv6Neighbor':
        entity = InterfaceIpv6Neighbor.from_dict(dict_buffer)
    return entity

def get_entity_class_object_by_type_v2(dict_buffer: dict):
    type = dict_buffer['type']
    type_name = globals().get(type)
    if type_name and hasattr(type_name, 'from_dict'): 
        entity = type_name.from_dict(dict_buffer)
    else:
        raise ValueError(f"Unknown type '{type}' or missing 'from_dict' method")
    return entity
...
while True:
    for message in consumer:
        msg_key_size = len(message.key) if message.key else 0
        msg_value_size = len(message.value) if message.value else 0
        msg_size = msg_key_size + msg_value_size
        ...
        if event_time is not None:
            ...
            if current_time - start_interval >= THROUGHPUT_INTERVAL:
                current_datetime = datetime.datetime.now(datetime.timezone.utc)
                throughput_records = processed_count / (current_time - start_interval)
                throughput_bytes = msg_iteration_size / (current_time - start_interval)
                
                throughput_csv_data = [start_interval_datetime, current_datetime, throughput_records, throughput_bytes, current_time - start_interval]
                ...
                start_interval_datetime = current_datetime
