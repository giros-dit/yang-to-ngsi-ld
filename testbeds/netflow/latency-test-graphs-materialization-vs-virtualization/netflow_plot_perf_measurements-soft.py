...
df_materialization_parsing = pd.read_csv('performance_measurements_parsing_materialization-softflow.csv')
df_materialization_integration = pd.read_csv('performance_measurements_integration_materialization-softflow.csv')
...
df_virtualization_parsing = pd.read_csv('performance_measurements_parsing_virtualization-softflow.csv')
df_virtualization_integration = pd.read_csv('performance_measurements_integration_virtualization-softflow.csv')
...
plot_materialization_parsing = df_materialization_parsing.boxplot(column = ['iteration_execution_time'], vert=True, patch_artist=True, showfliers = False, medianprops = dict(color = "red", linewidth = 1.5), boxprops = dict(facecolor = "blue"), ax=ax, positions=[1])
plot_virtualization_parsing = df_virtualization_parsing.boxplot(column = ['iteration_execution_time'], vert=True, patch_artist=True, showfliers = False, medianprops = dict(color = "yellow", linewidth = 1.5), boxprops = dict(facecolor = "grey"), ax=ax, positions=[2])
...
plt.savefig("notification_parsing_performance_measurements_data_integration-softflow.png", format="png", dpi=1500)

fig, ax = plt.subplots()

plot_materialization_integration = df_materialization_integration.boxplot(column = ['iteration_execution_time'], vert=True, patch_artist=True, showfliers = False, medianprops = dict(color = "red", linewidth = 1.5), boxprops = dict(facecolor = "blue"), ax=ax, positions=[1])
plot_virtualization_integration = df_virtualization_integration.boxplot(column = ['iteration_execution_time'], vert=True, patch_artist=True, showfliers = False, medianprops = dict(color = "yellow", linewidth = 1.5), boxprops = dict(facecolor = "grey"), ax=ax, positions=[2])

plt.xticks([1, 2], ['materialization', 'virtualization_CS'])
plt.xlabel('Data integration approach')
plt.ylabel('Latency (milliseconds)')
plt.title('Mean integration time per notification in \n NetFlow v9 telemetry')
plt.savefig("notification_integration_performance_measurements_data_integration-softflow.png", format="png", dpi=1500)