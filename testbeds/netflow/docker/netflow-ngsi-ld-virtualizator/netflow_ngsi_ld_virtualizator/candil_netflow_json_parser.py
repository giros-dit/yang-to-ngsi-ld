...
def parse_netflow(message, parsing_datetime):
    dict_buffers = []
    data = message.value
    timestamp = data["netflow-v9:netflow"]["collector-goflow2"]["time-received"]
    ##timestamp_sec = int(timestamp) / 1000.0
    timestamp_sec = int(timestamp) / 1000000.0
    datetime_ms = datetime.fromtimestamp(timestamp_sec)
    ##observed_at = str(datetime_ms)[:-3] + 'Z'
    observed_at = parsing_datetime.strftime("%Y-%m-%dT%H:%M:%S.%fZ") #observed_at = str(datetime_ms) + 'Z'
    #current_datetime = datetime.datetime.now(datetime.timezone.utc)
    #observed_at = current_datetime.strftime("%Y-%m-%dT%H:%M:%S.%fZ")
