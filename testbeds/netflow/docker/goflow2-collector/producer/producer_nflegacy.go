...
import (
	...
	"goflow2-collector/decoders/netflowlegacy"
...
func ConvertNetFlowLegacyRecord(baseTime uint32, uptime uint32, record netflowlegacy.RecordsNetFlowV5) *flowmessage.FlowMessage {
	...
	flowMessage.TimeFlowStartNs = uint64(baseTime)*1000000000 - uint64(timeDiffFirst)*1000000
	...
	flowMessage.TimeFlowEndNs = uint64(baseTime)*1000000000 - uint64(timeDiffLast)*1000000
	...
	flowMessage.SrcTos = uint32(record.Tos)
	...
	flowMessage.PacketsIn = uint64(record.DPkts)
	flowMessage.BytesIn = uint64(record.DOctets)
