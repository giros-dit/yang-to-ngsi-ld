...
func ConvertNetFlowDataSet(version uint16, baseTime uint32, uptime uint32, record []netflow.DataField, mapperNetFlow *NetFlowMapper, mapperSFlow *SFlowMapper) *flowmessage.FlowMessage {
	...
	for i := range record {
		...
		case netflow.NFV9_FIELD_IN_BYTES:
			DecodeUNumber(v, &(flowMessage.BytesIn))
		case netflow.NFV9_FIELD_IN_PKTS:
			DecodeUNumber(v, &(flowMessage.PacketsIn))
		case netflow.NFV9_FIELD_OUT_BYTES:
			DecodeUNumber(v, &(flowMessage.BytesOut))
		case netflow.NFV9_FIELD_OUT_PKTS:
			DecodeUNumber(v, &(flowMessage.PacketsOut))
		...
		// Engine
		case netflow.NFV9_FIELD_ENGINE_ID:
			DecodeUNumber(v, &(flowMessage.EngineId))
		case netflow.NFV9_FIELD_ENGINE_TYPE:
			DecodeUNumber(v, &(flowMessage.EngineType))
		...
		case netflow.NFV9_FIELD_SRC_TOS:
			DecodeUNumber(v, &(flowMessage.SrcTos))
		case netflow.NFV9_FIELD_DST_TOS:
			DecodeUNumber(v, &(flowMessage.DstTos))
		case netflow.NFV9_FIELD_TCP_FLAGS:
			DecodeUNumber(v, &(flowMessage.TCPFlags))
		case netflow.NFV9_FIELD_MIN_TTL:
			DecodeUNumber(v, &(flowMessage.MinTTL))
		case netflow.NFV9_FIELD_MAX_TTL:
			DecodeUNumber(v, &(flowMessage.MaxTTL))
		...
		case netflow.NFV9_FIELD_IPV4_SRC_PREFIX:
			flowMessage.SrcPrefix = v
			flowMessage.Etype = 0x800
		case netflow.NFV9_FIELD_IPV4_DST_PREFIX:
			flowMessage.DstPrefix = v
			flowMessage.Etype = 0x800
		...
		// MPLS
		case netflow.NFV9_FIELD_MPLS_PAL_RD:
			DecodeUNumber(v, &(flowMessage.MPLSPalRd))
		case netflow.NFV9_FIELD_MPLS_PREFIX_LEN:
			DecodeUNumber(v, &(flowMessage.MPLSPrefixLen))
		case netflow.NFV9_FIELD_MPLS_TOP_LABEL_TYPE:
			DecodeUNumber(v, &(flowMessage.MPLSTopLabelType))
		case netflow.NFV9_FIELD_MPLS_TOP_LABEL_IP_ADDR:
			flowMessage.MPLSTopLabelIP = v
		case netflow.NFV9_FIELD_MPLS_LABEL_1:
			DecodeUNumber(v, &(flowMessage.MPLS1Label))
		case netflow.NFV9_FIELD_MPLS_LABEL_2:
			DecodeUNumber(v, &(flowMessage.MPLS2Label))
		case netflow.NFV9_FIELD_MPLS_LABEL_3:
			DecodeUNumber(v, &(flowMessage.MPLS3Label))
		case netflow.NFV9_FIELD_MPLS_LABEL_4:
			DecodeUNumber(v, &(flowMessage.MPLS4Label))
		case netflow.NFV9_FIELD_MPLS_LABEL_5:
			DecodeUNumber(v, &(flowMessage.MPLS5Label))
		case netflow.NFV9_FIELD_MPLS_LABEL_6:
			DecodeUNumber(v, &(flowMessage.MPLS6Label))
		case netflow.NFV9_FIELD_MPLS_LABEL_7:
			DecodeUNumber(v, &(flowMessage.MPLS7Label))
		case netflow.NFV9_FIELD_MPLS_LABEL_8:
			DecodeUNumber(v, &(flowMessage.MPLS8Label))
		case netflow.NFV9_FIELD_MPLS_LABEL_9:
			DecodeUNumber(v, &(flowMessage.MPLS9Label))
		case netflow.NFV9_FIELD_MPLS_LABEL_10:
			DecodeUNumber(v, &(flowMessage.MPLS10Label))
		...
		case netflow.NFV9_FIELD_IN_SRC_MAC:
			DecodeUNumber(v, &(flowMessage.SrcMacIn))
		case netflow.NFV9_FIELD_IN_DST_MAC:
			DecodeUNumber(v, &(flowMessage.DstMacIn))
		case netflow.NFV9_FIELD_OUT_SRC_MAC:
			DecodeUNumber(v, &(flowMessage.SrcMacOut))
		case netflow.NFV9_FIELD_OUT_DST_MAC:
			DecodeUNumber(v, &(flowMessage.DstMacOut))
		...
		case netflow.NFV9_FIELD_IPV6_OPTION_HEADERS:
			DecodeUNumber(v, &(flowMessage.IPv6OptionHeaders))
		...
		default:
			if version == 9 {
				...
				case netflow.NFV9_FIELD_FIRST_SWITCHED:
					...
					flowMessage.TimeFlowStartNs = uint64(baseTime)*1000000000 - uint64(timeDiff)*1000000
				case netflow.NFV9_FIELD_LAST_SWITCHED:
					...
					flowMessage.TimeFlowEndNs = uint64(baseTime)*1000000000 - uint64(timeDiff)*1000000
				}
			} else if version == 10 {
				switch df.Type {
				...
				case netflow.IPFIX_FIELD_flowStartDeltaMicroseconds:
					...
					flowMessage.TimeFlowStartNs = uint64(baseTime)*1000000000 - time*1000
				case netflow.IPFIX_FIELD_flowEndDeltaMicroseconds:
					...
					flowMessage.TimeFlowEndNs = uint64(baseTime)*1000000000 - time*1000
				// RFC7133
				case netflow.IPFIX_FIELD_dataLinkFrameSize:
					DecodeUNumber(v, &(flowMessage.BytesIn))
					flowMessage.PacketsIn = 1
				case netflow.IPFIX_FIELD_dataLinkFrameSection:
					ParseEthernetHeader(flowMessage, v, mapperSFlow)
					flowMessage.PacketsIn = 1
					if flowMessage.BytesIn == 0 {
						flowMessage.BytesIn = uint64(len(v))
					}
				}
			}
		}
...
func ProcessMessageNetFlowConfig(msgDec interface{}, samplingRateSys SamplingRateSystem, config *ProducerConfigMapped) ([]*flowmessage.FlowMessage, error) {
	...
	case netflow.NFv9Packet:
		...
		for _, fmsg := range flowMessageSet {
			...
			fmsg.SourceId = obsDomainId
			fmsg.UnixSeconds = baseTime
			fmsg.Count = count
			fmsg.SystemUptime = uptime
			fmsg.SamplingRate = uint32(samplingRate)
		}
