...
class ForwardErrorCorrection(BaseModel):
    """
    ...
    def value_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('disabled', 'rs-528', 'rs-544', 'base-r', 'rs-108'):
            raise ValueError("must be one of enum values ('disabled', 'rs-528', 'rs-544', 'base-r', 'rs-108')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in ForwardErrorCorrection) in the input: " + _key)
