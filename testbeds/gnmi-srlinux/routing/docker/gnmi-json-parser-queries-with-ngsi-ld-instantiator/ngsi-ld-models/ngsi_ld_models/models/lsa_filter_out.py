...
class LsaFilterOut(BaseModel):
    ...
    def value_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['none', 'all', 'except-own-rtrlsa', 'except-own-rtrlsa-and-defaults']):
            raise ValueError("must be one of enum values ('none', 'all', 'except-own-rtrlsa', 'except-own-rtrlsa-and-defaults')")
        return value
