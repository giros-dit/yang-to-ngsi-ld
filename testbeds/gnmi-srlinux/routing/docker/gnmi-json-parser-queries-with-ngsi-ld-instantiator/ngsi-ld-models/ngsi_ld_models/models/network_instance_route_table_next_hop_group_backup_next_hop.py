...
class NetworkInstanceRouteTableNextHopGroupBackupNextHop(BaseModel):
    """
    List of backup next-hops associated with the NHG  YANG module: srl_nokia-ip-route-tables.yang 
    """ # noqa: E501
    id: Optional[NetworkInstanceRouteTableNextHopGroupBackupNextHopId] = None
    type: StrictStr = Field(description="NGSI-LD Entity identifier. It has to be NetworkInstanceRouteTableNextHopGroupBackupNextHop.")
    ...
    next_hop: Optional[NetworkInstanceRouteTableNextHopGroupBackupNextHopNextHop] = Field(default=None, alias="nextHop")
    resolved: Optional[NetworkInstanceRouteTableNextHopGroupBackupNextHopResolved] = None
    resource_allocation_failed: Optional[NetworkInstanceRouteTableNextHopGroupBackupNextHopResourceAllocationFailed] = Field(default=None, alias="resourceAllocationFailed")
    is_part_of: IsPartOf = Field(alias="isPartOf")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["id", "type", "scope", "location", "observationSpace", "operationSpace", "createdAt", "modifiedAt", "deletedAt", "nextHop", "resolved", "resourceAllocationFailed", "isPartOf"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['NetworkInstanceRouteTableNextHopGroupBackupNextHop']):
            raise ValueError("must be one of enum values ('NetworkInstanceRouteTableNextHopGroupBackupNextHop')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "id": NetworkInstanceRouteTableNextHopGroupBackupNextHopId.from_dict(obj["id"]) if obj.get("id") is not None else None,
            "type": obj.get("type") if obj.get("type") is not None else 'NetworkInstanceRouteTableNextHopGroupBackupNextHop',
            ...
            "nextHop": NetworkInstanceRouteTableNextHopGroupBackupNextHopNextHop.from_dict(obj["nextHop"]) if obj.get("nextHop") is not None else None,
            "resolved": NetworkInstanceRouteTableNextHopGroupBackupNextHopResolved.from_dict(obj["resolved"]) if obj.get("resolved") is not None else None,
            "resourceAllocationFailed": NetworkInstanceRouteTableNextHopGroupBackupNextHopResourceAllocationFailed.from_dict(obj["resourceAllocationFailed"]) if obj.get("resourceAllocationFailed") is not None else None,
