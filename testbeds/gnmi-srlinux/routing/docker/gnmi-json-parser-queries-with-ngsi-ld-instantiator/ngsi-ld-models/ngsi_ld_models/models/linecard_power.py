...
class LinecardPower(BaseModel):
    ...
    type: StrictStr = Field(description="NGSI-LD Entity identifier. It has to be LinecardPower.")
    ...
    allocated: Optional[LinecardPowerAllocated] = None
    used: Optional[LinecardPowerUsed] = None
    required: Optional[LinecardPowerRequired] = None
    is_part_of: IsPartOf = Field(alias="isPartOf")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["id", "type", "scope", "location", "observationSpace", "operationSpace", "createdAt", "modifiedAt", "deletedAt", "allocated", "used", "required", "isPartOf"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['LinecardPower']):
            raise ValueError("must be one of enum values ('LinecardPower')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "id": obj.get("id"),
            "type": obj.get("type") if obj.get("type") is not None else 'LinecardPower',
            ...
            "allocated": LinecardPowerAllocated.from_dict(obj["allocated"]) if obj.get("allocated") is not None else None,
            "used": LinecardPowerUsed.from_dict(obj["used"]) if obj.get("used") is not None else None,
            "required": LinecardPowerRequired.from_dict(obj["required"]) if obj.get("required") is not None else None,
