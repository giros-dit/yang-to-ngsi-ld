...
class LinecardForwardingComplexControlPlaneTraffic(BaseModel):
    """
    ...
    type: StrictStr = Field(description="NGSI-LD Entity identifier. It has to be LinecardForwardingComplexControlPlaneTraffic.")
    ...
    queued_aggregate: Optional[QueuedAggregate] = Field(default=None, alias="queuedAggregate")
    queued_bytes_aggregate: Optional[QueuedBytesAggregate] = Field(default=None, alias="queuedBytesAggregate")
    dropped_aggregate: Optional[DroppedAggregate] = Field(default=None, alias="droppedAggregate")
    dropped_bytes_aggregate: Optional[DroppedBytesAggregate] = Field(default=None, alias="droppedBytesAggregate")
    is_part_of: IsPartOf = Field(alias="isPartOf")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["id", "type", "scope", "location", "observationSpace", "operationSpace", "createdAt", "modifiedAt", "deletedAt", "queuedAggregate", "queuedBytesAggregate", "droppedAggregate", "droppedBytesAggregate", "isPartOf"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['LinecardForwardingComplexControlPlaneTraffic']):
            raise ValueError("must be one of enum values ('LinecardForwardingComplexControlPlaneTraffic')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # override the default output from pydantic by calling `to_dict()` of queued_aggregate
        if self.queued_aggregate:
            _dict['queuedAggregate'] = self.queued_aggregate.to_dict()
        # override the default output from pydantic by calling `to_dict()` of queued_bytes_aggregate
        if self.queued_bytes_aggregate:
            _dict['queuedBytesAggregate'] = self.queued_bytes_aggregate.to_dict()
        # override the default output from pydantic by calling `to_dict()` of dropped_aggregate
        if self.dropped_aggregate:
            _dict['droppedAggregate'] = self.dropped_aggregate.to_dict()
        # override the default output from pydantic by calling `to_dict()` of dropped_bytes_aggregate
        if self.dropped_bytes_aggregate:
            _dict['droppedBytesAggregate'] = self.dropped_bytes_aggregate.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "id": obj.get("id"),
            "type": obj.get("type") if obj.get("type") is not None else 'LinecardForwardingComplexControlPlaneTraffic',
            ...
            "queuedAggregate": QueuedAggregate.from_dict(obj["queuedAggregate"]) if obj.get("queuedAggregate") is not None else None,
            "queuedBytesAggregate": QueuedBytesAggregate.from_dict(obj["queuedBytesAggregate"]) if obj.get("queuedBytesAggregate") is not None else None,
            "droppedAggregate": DroppedAggregate.from_dict(obj["droppedAggregate"]) if obj.get("droppedAggregate") is not None else None,
            "droppedBytesAggregate": DroppedBytesAggregate.from_dict(obj["droppedBytesAggregate"]) if obj.get("droppedBytesAggregate") is not None else None,
