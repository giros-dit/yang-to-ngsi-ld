...
class RisingThresholdAction(BaseModel):
    """
    ...
    def value_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['none', 'trigger-event', 'disable-interface']):
            raise ValueError("must be one of enum values ('none', 'trigger-event', 'disable-interface')")
        return value
