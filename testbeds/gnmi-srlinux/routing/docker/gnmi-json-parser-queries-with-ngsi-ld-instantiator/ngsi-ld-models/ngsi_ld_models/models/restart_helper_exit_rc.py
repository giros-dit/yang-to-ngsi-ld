...
class RestartHelperExitRc(BaseModel):
    """
    ...
    def value_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['none', 'in-progress', 'completed', 'timed-out', 'topology-changed', 'bfd-down']):
            raise ValueError("must be one of enum values ('none', 'in-progress', 'completed', 'timed-out', 'topology-changed', 'bfd-down')")
        return value
