...
class PolicyDefaultActionPolicyResult(BaseModel):
    """
    ...
    def value_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['accept', 'reject', 'next-policy']):
            raise ValueError("must be one of enum values ('accept', 'reject', 'next-policy')")
        return value
