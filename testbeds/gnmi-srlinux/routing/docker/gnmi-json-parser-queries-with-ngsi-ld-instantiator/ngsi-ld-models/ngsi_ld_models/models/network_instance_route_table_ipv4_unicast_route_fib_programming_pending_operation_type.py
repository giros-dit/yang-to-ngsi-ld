...
class NetworkInstanceRouteTableIpv4UnicastRouteFibProgrammingPendingOperationType(BaseModel):
    """
    ...
    def value_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['add', 'delete', 'modify', 'none']):
            raise ValueError("must be one of enum values ('add', 'delete', 'modify', 'none')")
        return value
