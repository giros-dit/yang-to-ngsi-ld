...
class NetworkInstanceNextHopGroupsGroupNexthopTunnelResolutionMode(BaseModel):
    """
    ...
    def value_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['prefer', 'require', 'disabled']):
            raise ValueError("must be one of enum values ('prefer', 'require', 'disabled')")
        return value
