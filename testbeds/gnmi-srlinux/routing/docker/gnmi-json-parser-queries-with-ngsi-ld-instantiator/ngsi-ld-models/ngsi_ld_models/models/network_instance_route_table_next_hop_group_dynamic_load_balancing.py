...
class NetworkInstanceRouteTableNextHopGroupDynamicLoadBalancing(BaseModel):
    """
     YANG module: srl_nokia-ip-route-tables.yang 
    """ # noqa: E501
    id: Optional[StrictStr] = Field(default=None, description="Entity id. ")
    type: StrictStr = Field(description="NGSI-LD Entity identifier. It has to be NetworkInstanceRouteTableNextHopGroupDynamicLoadBalancing.")
    ...
    enabled: Optional[NetworkInstanceRouteTableNextHopGroupDynamicLoadBalancingEnabled] = None
    flows_rebalanced: Optional[FlowsRebalanced] = Field(default=None, alias="flowsRebalanced")
    is_part_of: IsPartOf = Field(alias="isPartOf")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["id", "type", "scope", "location", "observationSpace", "operationSpace", "createdAt", "modifiedAt", "deletedAt", "requested", "enabled", "flowsRebalanced", "isPartOf"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['NetworkInstanceRouteTableNextHopGroupDynamicLoadBalancing']):
            raise ValueError("must be one of enum values ('NetworkInstanceRouteTableNextHopGroupDynamicLoadBalancing')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # override the default output from pydantic by calling `to_dict()` of flows_rebalanced
        if self.flows_rebalanced:
            _dict['flowsRebalanced'] = self.flows_rebalanced.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "id": obj.get("id"),
            "type": obj.get("type") if obj.get("type") is not None else 'NetworkInstanceRouteTableNextHopGroupDynamicLoadBalancing',
            ...
            "requested": Requested.from_dict(obj["requested"]) if obj.get("requested") is not None else None,
            "enabled": NetworkInstanceRouteTableNextHopGroupDynamicLoadBalancingEnabled.from_dict(obj["enabled"]) if obj.get("enabled") is not None else None,
            "flowsRebalanced": FlowsRebalanced.from_dict(obj["flowsRebalanced"]) if obj.get("flowsRebalanced") is not None else None,
