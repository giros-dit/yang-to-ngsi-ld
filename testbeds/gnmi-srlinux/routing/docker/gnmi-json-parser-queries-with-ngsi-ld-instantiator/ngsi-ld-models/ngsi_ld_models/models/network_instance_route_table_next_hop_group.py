...
class NetworkInstanceRouteTableNextHopGroup(BaseModel):
    """
     YANG module: srl_nokia-ip-route-tables.yang 
    """ # noqa: E501
    id: Optional[StrictStr] = Field(default=None, description="Entity id. ")
    type: StrictStr = Field(description="NGSI-LD Entity identifier. It has to be NetworkInstanceRouteTableNextHopGroup.")
    ...
    index: Optional[NetworkInstanceRouteTableNextHopGroupIndex] = None
    group_name_alias: Optional[GroupNameAlias] = Field(default=None, alias="groupNameAlias")
    programmed_index: Optional[NetworkInstanceRouteTableNextHopGroupProgrammedIndex] = Field(default=None, alias="programmedIndex")
    backup_next_hop_group: Optional[NetworkInstanceRouteTableNextHopGroupBackupNextHopGroup] = Field(default=None, alias="backupNextHopGroup")
    backup_active: Optional[NetworkInstanceRouteTableNextHopGroupBackupActive] = Field(default=None, alias="backupActive")
    is_part_of: IsPartOf = Field(alias="isPartOf")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["id", "type", "scope", "location", "observationSpace", "operationSpace", "createdAt", "modifiedAt", "deletedAt", "index", "groupNameAlias", "programmedIndex", "backupNextHopGroup", "backupActive", "isPartOf"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['NetworkInstanceRouteTableNextHopGroup']):
            raise ValueError("must be one of enum values ('NetworkInstanceRouteTableNextHopGroup')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # override the default output from pydantic by calling `to_dict()` of group_name_alias
        if self.group_name_alias:
            _dict['groupNameAlias'] = self.group_name_alias.to_dict()
        ...
        # override the default output from pydantic by calling `to_dict()` of backup_next_hop_group
        if self.backup_next_hop_group:
            _dict['backupNextHopGroup'] = self.backup_next_hop_group.to_dict()
        # override the default output from pydantic by calling `to_dict()` of backup_active
        if self.backup_active:
            _dict['backupActive'] = self.backup_active.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "id": obj.get("id"),
            "type": obj.get("type") if obj.get("type") is not None else 'NetworkInstanceRouteTableNextHopGroup',
            ...
            "index": NetworkInstanceRouteTableNextHopGroupIndex.from_dict(obj["index"]) if obj.get("index") is not None else None,
            "groupNameAlias": GroupNameAlias.from_dict(obj["groupNameAlias"]) if obj.get("groupNameAlias") is not None else None,
            "programmedIndex": NetworkInstanceRouteTableNextHopGroupProgrammedIndex.from_dict(obj["programmedIndex"]) if obj.get("programmedIndex") is not None else None,
            "backupNextHopGroup": NetworkInstanceRouteTableNextHopGroupBackupNextHopGroup.from_dict(obj["backupNextHopGroup"]) if obj.get("backupNextHopGroup") is not None else None,
            "backupActive": NetworkInstanceRouteTableNextHopGroupBackupActive.from_dict(obj["backupActive"]) if obj.get("backupActive") is not None else None,
