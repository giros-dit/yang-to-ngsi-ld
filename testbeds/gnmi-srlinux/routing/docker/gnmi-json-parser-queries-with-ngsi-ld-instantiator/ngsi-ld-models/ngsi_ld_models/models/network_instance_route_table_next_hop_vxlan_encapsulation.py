...
class NetworkInstanceRouteTableNextHopVxlanEncapsulation(BaseModel):
    """
     YANG module: srl_nokia-ip-route-tables.yang 
    """ # noqa: E501
    id: Optional[StrictStr] = Field(default=None, description="Entity id. ")
    type: StrictStr = Field(description="NGSI-LD Entity identifier. It has to be NetworkInstanceRouteTableNextHopVxlanEncapsulation.")
    ...
    vni: Optional[Vni] = None
    interface: Optional[NetworkInstanceRouteTableNextHopVxlanEncapsulationInterface] = None
    destination_mac: Optional[DestinationMac] = Field(default=None, alias="destinationMac")
    is_part_of: IsPartOf = Field(alias="isPartOf")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["id", "type", "scope", "location", "observationSpace", "operationSpace", "createdAt", "modifiedAt", "deletedAt", "vni", "interface", "destinationMac", "isPartOf"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['NetworkInstanceRouteTableNextHopVxlanEncapsulation']):
            raise ValueError("must be one of enum values ('NetworkInstanceRouteTableNextHopVxlanEncapsulation')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # override the default output from pydantic by calling `to_dict()` of vni
        if self.vni:
            _dict['vni'] = self.vni.to_dict()
        # override the default output from pydantic by calling `to_dict()` of interface
        if self.interface:
            _dict['interface'] = self.interface.to_dict()
        # override the default output from pydantic by calling `to_dict()` of destination_mac
        if self.destination_mac:
            _dict['destinationMac'] = self.destination_mac.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "id": obj.get("id"),
            "type": obj.get("type") if obj.get("type") is not None else 'NetworkInstanceRouteTableNextHopVxlanEncapsulation',
            ...
            "vni": Vni.from_dict(obj["vni"]) if obj.get("vni") is not None else None,
            "interface": NetworkInstanceRouteTableNextHopVxlanEncapsulationInterface.from_dict(obj["interface"]) if obj.get("interface") is not None else None,
            "destinationMac": DestinationMac.from_dict(obj["destinationMac"]) if obj.get("destinationMac") is not None else None,
