...
class PolicyStatementActionPolicyResult(BaseModel):
    """
    ...
    def value_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['accept', 'reject', 'next-statement', 'next-policy']):
            raise ValueError("must be one of enum values ('accept', 'reject', 'next-statement', 'next-policy')")
        return value
