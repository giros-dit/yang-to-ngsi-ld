...
class NetworkInstanceProtocolsOspfInstanceTimersSpfWait(BaseModel):
    """
    Enter the spf-wait context  YANG module: srl_nokia-ospf.yang 
    """ # noqa: E501
    id: Optional[StrictStr] = Field(default=None, description="Entity id. ")
    type: StrictStr = Field(description="NGSI-LD Entity identifier. It has to be NetworkInstanceProtocolsOspfInstanceTimersSpfWait.")
    ...
    spf_max_wait: Optional[SpfMaxWait] = Field(default=None, alias="spfMaxWait")
    spf_initial_wait: Optional[SpfInitialWait] = Field(default=None, alias="spfInitialWait")
    spf_second_wait: Optional[SpfSecondWait] = Field(default=None, alias="spfSecondWait")
    is_part_of: IsPartOf = Field(alias="isPartOf")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["id", "type", "scope", "location", "observationSpace", "operationSpace", "createdAt", "modifiedAt", "deletedAt", "spfMaxWait", "spfInitialWait", "spfSecondWait", "isPartOf"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['NetworkInstanceProtocolsOspfInstanceTimersSpfWait']):
            raise ValueError("must be one of enum values ('NetworkInstanceProtocolsOspfInstanceTimersSpfWait')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # override the default output from pydantic by calling `to_dict()` of spf_max_wait
        if self.spf_max_wait:
            _dict['spfMaxWait'] = self.spf_max_wait.to_dict()
        # override the default output from pydantic by calling `to_dict()` of spf_initial_wait
        if self.spf_initial_wait:
            _dict['spfInitialWait'] = self.spf_initial_wait.to_dict()
        # override the default output from pydantic by calling `to_dict()` of spf_second_wait
        if self.spf_second_wait:
            _dict['spfSecondWait'] = self.spf_second_wait.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "id": obj.get("id"),
            "type": obj.get("type") if obj.get("type") is not None else 'NetworkInstanceProtocolsOspfInstanceTimersSpfWait',
            ...
            "spfMaxWait": SpfMaxWait.from_dict(obj["spfMaxWait"]) if obj.get("spfMaxWait") is not None else None,
            "spfInitialWait": SpfInitialWait.from_dict(obj["spfInitialWait"]) if obj.get("spfInitialWait") is not None else None,
            "spfSecondWait": SpfSecondWait.from_dict(obj["spfSecondWait"]) if obj.get("spfSecondWait") is not None else None,
