...
class NetworkInstanceNextHopGroupsGroup(BaseModel):
    """
    ...
    type: StrictStr = Field(description="NGSI-LD Entity identifier. It has to be NetworkInstanceNextHopGroupsGroup.")
    ...
    name: Optional[NetworkInstanceNextHopGroupsGroupName] = None
    admin_state: Optional[NetworkInstanceNextHopGroupsGroupAdminState] = Field(default=None, alias="adminState")
    backup_next_hop_group: Optional[NetworkInstanceNextHopGroupsGroupBackupNextHopGroup] = Field(default=None, alias="backupNextHopGroup")
    algorithm: Optional[NetworkInstanceNextHopGroupsGroupAlgorithm] = None
    is_part_of: IsPartOf = Field(alias="isPartOf")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["id", "type", "scope", "location", "observationSpace", "operationSpace", "createdAt", "modifiedAt", "deletedAt", "name", "adminState", "backupNextHopGroup", "algorithm", "isPartOf"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['NetworkInstanceNextHopGroupsGroup']):
            raise ValueError("must be one of enum values ('NetworkInstanceNextHopGroupsGroup')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "id": obj.get("id"),
            "type": obj.get("type") if obj.get("type") is not None else 'NetworkInstanceNextHopGroupsGroup',
            ...
            "name": NetworkInstanceNextHopGroupsGroupName.from_dict(obj["name"]) if obj.get("name") is not None else None,
            "adminState": NetworkInstanceNextHopGroupsGroupAdminState.from_dict(obj["adminState"]) if obj.get("adminState") is not None else None,
            "backupNextHopGroup": NetworkInstanceNextHopGroupsGroupBackupNextHopGroup.from_dict(obj["backupNextHopGroup"]) if obj.get("backupNextHopGroup") is not None else None,
            "algorithm": NetworkInstanceNextHopGroupsGroupAlgorithm.from_dict(obj["algorithm"]) if obj.get("algorithm") is not None else None,
