...
class ControlPower(BaseModel):
    ...
    type: StrictStr = Field(description="NGSI-LD Entity identifier. It has to be ControlPower.")
    ...
    allocated: Optional[ControlPowerAllocated] = None
    used: Optional[ControlPowerUsed] = None
    required: Optional[ControlPowerRequired] = None
    ...
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['ControlPower']):
            raise ValueError("must be one of enum values ('ControlPower')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "id": obj.get("id"),
            "type": obj.get("type") if obj.get("type") is not None else 'ControlPower',
            ...
            "allocated": ControlPowerAllocated.from_dict(obj["allocated"]) if obj.get("allocated") is not None else None,
            "used": ControlPowerUsed.from_dict(obj["used"]) if obj.get("used") is not None else None,
            "required": ControlPowerRequired.from_dict(obj["required"]) if obj.get("required") is not None else None,
