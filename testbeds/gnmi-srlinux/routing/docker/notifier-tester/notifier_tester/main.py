...
async def startup_event():
    ...
    for entity in LIST_ENTITIES:
        ...
        subs_request = CreateSubscriptionRequest (
            ...
            description="On-change subscription to {0} entities.".format(entity),
            #watchedAttributes=[""],
            notification=notification_params
        )
...
async def receiveNotification(request: Request):
    ...
    for entity in notification["data"]:
        if entity["type"] == "NetworkInstanceProtocolsOspfInstance":
            logger.info("Entity notification: %s\n" % entity)
            if "observedAt" in entity["adminState"] and "modifiedAt" in entity["adminState"]:

                observed_at = parser.parse(entity["adminState"]["observedAt"])
                modified_at = parser.parse(entity["adminState"]["modifiedAt"])
        ...
        elif entity["type"] == "NetworkInstanceIcmpStatisticsTotal":
            logger.info("Entity notification: %s\n" % entity)
            if "observedAt" in entity["inPackets"] and "modifiedAt" in entity["inPackets"]:

                observed_at = parser.parse(entity["inPackets"]["observedAt"])
                modified_at = parser.parse(entity["inPackets"]["modifiedAt"])
