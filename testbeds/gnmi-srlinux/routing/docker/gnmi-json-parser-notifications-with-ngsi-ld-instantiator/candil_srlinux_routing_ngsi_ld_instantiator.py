...
from candil_srlinux_routing_json_parser_notifications import parse_gnmi_notification

import ngsi_ld_client

# Complete import sequences of Python classes for managing NGSI-LD modeled data
...
# Complete checks of NGSI-LD types
def get_entity_class_object_by_type(dict_buffer: dict):
    type = dict_buffer['type']
    if type == 'NetworkInstanceIcmpStatistics':
        entity = NetworkInstanceIcmpStatistics.from_dict(dict_buffer)
    else:
        entity = None
    return entity
...
print("I will consume messages (gNMI notifications) from a Kafka topic named routing-state-notifications")

consumer = KafkaConsumer('routing-state-notifications', bootstrap_servers=['kafka:9092'], auto_offset_reset='latest', value_deserializer=lambda x: json.loads(x.decode('utf-8')))
