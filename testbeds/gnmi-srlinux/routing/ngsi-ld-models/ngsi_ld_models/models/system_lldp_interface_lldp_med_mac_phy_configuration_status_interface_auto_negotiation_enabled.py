...
class SystemLldpInterfaceLldpMedMacPhyConfigurationStatusInterfaceAutoNegotiationEnabled(BaseModel):
    """
    ...
    def value_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['unknown', 'true', 'false', 'tlv-not-enabled']):
            raise ValueError("must be one of enum values ('unknown', 'true', 'false', 'tlv-not-enabled')")
        return value
