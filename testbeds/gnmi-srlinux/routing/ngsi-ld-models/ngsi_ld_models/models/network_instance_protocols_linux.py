...
class NetworkInstanceProtocolsLinux(BaseModel):
    """
    ...
    type: StrictStr = Field(description="NGSI-LD Entity identifier. It has to be NetworkInstanceProtocolsLinux.")
    ...
    import_routes: Optional[ImportRoutes] = Field(default=None, alias="importRoutes")
    export_routes: Optional[ExportRoutes] = Field(default=None, alias="exportRoutes")
    export_neighbors: Optional[ExportNeighbors] = Field(default=None, alias="exportNeighbors")
    is_part_of: IsPartOf = Field(alias="isPartOf")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["id", "type", "scope", "location", "observationSpace", "operationSpace", "createdAt", "modifiedAt", "deletedAt", "importRoutes", "exportRoutes", "exportNeighbors", "isPartOf"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['NetworkInstanceProtocolsLinux']):
            raise ValueError("must be one of enum values ('NetworkInstanceProtocolsLinux')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # override the default output from pydantic by calling `to_dict()` of import_routes
        if self.import_routes:
            _dict['importRoutes'] = self.import_routes.to_dict()
        # override the default output from pydantic by calling `to_dict()` of export_routes
        if self.export_routes:
            _dict['exportRoutes'] = self.export_routes.to_dict()
        # override the default output from pydantic by calling `to_dict()` of export_neighbors
        if self.export_neighbors:
            _dict['exportNeighbors'] = self.export_neighbors.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "id": obj.get("id"),
            "type": obj.get("type") if obj.get("type") is not None else 'NetworkInstanceProtocolsLinux',
            ...
            "importRoutes": ImportRoutes.from_dict(obj["importRoutes"]) if obj.get("importRoutes") is not None else None,
            "exportRoutes": ExportRoutes.from_dict(obj["exportRoutes"]) if obj.get("exportRoutes") is not None else None,
            "exportNeighbors": ExportNeighbors.from_dict(obj["exportNeighbors"]) if obj.get("exportNeighbors") is not None else None,
