...
class NetworkInstanceTcpConnection(BaseModel):
    """
    ...
    type: StrictStr = Field(description="NGSI-LD Entity identifier. It has to be NetworkInstanceTcpConnection.")
    ...
    local_address: Optional[NetworkInstanceTcpConnectionLocalAddress] = Field(default=None, alias="localAddress")
    local_port: Optional[NetworkInstanceTcpConnectionLocalPort] = Field(default=None, alias="localPort")
    remote_address: Optional[RemoteAddress] = Field(default=None, alias="remoteAddress")
    remote_port: Optional[RemotePort] = Field(default=None, alias="remotePort")
    session_state: Optional[SessionState] = Field(default=None, alias="sessionState")
    process_id: Optional[NetworkInstanceTcpConnectionProcessId] = Field(default=None, alias="processId")
    is_part_of: IsPartOf = Field(alias="isPartOf")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["id", "type", "scope", "location", "observationSpace", "operationSpace", "createdAt", "modifiedAt", "deletedAt", "localAddress", "localPort", "remoteAddress", "remotePort", "sessionState", "processId", "isPartOf"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['NetworkInstanceTcpConnection']):
            raise ValueError("must be one of enum values ('NetworkInstanceTcpConnection')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        if self.local_address:
            _dict['localAddress'] = self.local_address.to_dict()
        # override the default output from pydantic by calling `to_dict()` of local_port
        if self.local_port:
            _dict['localPort'] = self.local_port.to_dict()
        # override the default output from pydantic by calling `to_dict()` of remote_address
        if self.remote_address:
            _dict['remoteAddress'] = self.remote_address.to_dict()
        # override the default output from pydantic by calling `to_dict()` of remote_port
        if self.remote_port:
            _dict['remotePort'] = self.remote_port.to_dict()
        # override the default output from pydantic by calling `to_dict()` of session_state
        if self.session_state:
            _dict['sessionState'] = self.session_state.to_dict()
        # override the default output from pydantic by calling `to_dict()` of process_id
        if self.process_id:
            _dict['processId'] = self.process_id.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "id": obj.get("id"),
            "type": obj.get("type") if obj.get("type") is not None else 'NetworkInstanceTcpConnection',
            ...
            "localAddress": NetworkInstanceTcpConnectionLocalAddress.from_dict(obj["localAddress"]) if obj.get("localAddress") is not None else None,
            "localPort": NetworkInstanceTcpConnectionLocalPort.from_dict(obj["localPort"]) if obj.get("localPort") is not None else None,
            "remoteAddress": RemoteAddress.from_dict(obj["remoteAddress"]) if obj.get("remoteAddress") is not None else None,
            "remotePort": RemotePort.from_dict(obj["remotePort"]) if obj.get("remotePort") is not None else None,
            "sessionState": SessionState.from_dict(obj["sessionState"]) if obj.get("sessionState") is not None else None,
            "processId": NetworkInstanceTcpConnectionProcessId.from_dict(obj["processId"]) if obj.get("processId") is not None else None,
