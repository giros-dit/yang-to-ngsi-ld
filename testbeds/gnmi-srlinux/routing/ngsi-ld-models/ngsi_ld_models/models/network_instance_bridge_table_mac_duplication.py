...
class NetworkInstanceBridgeTableMacDuplication(BaseModel):
    """
    ...
    type: StrictStr = Field(description="NGSI-LD Entity identifier. It has to be NetworkInstanceBridgeTableMacDuplication.")
    ...
    admin_state: Optional[NetworkInstanceBridgeTableMacDuplicationAdminState] = Field(default=None, alias="adminState")
    monitoring_window: Optional[MonitoringWindow] = Field(default=None, alias="monitoringWindow")
    num_moves: Optional[NumMoves] = Field(default=None, alias="numMoves")
    hold_down_time: Optional[HoldDownTime] = Field(default=None, alias="holdDownTime")
    action: Optional[NetworkInstanceBridgeTableMacDuplicationAction] = None
    is_part_of: IsPartOf = Field(alias="isPartOf")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["id", "type", "scope", "location", "observationSpace", "operationSpace", "createdAt", "modifiedAt", "deletedAt", "adminState", "monitoringWindow", "numMoves", "holdDownTime", "action", "isPartOf"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['NetworkInstanceBridgeTableMacDuplication']):
            raise ValueError("must be one of enum values ('NetworkInstanceBridgeTableMacDuplication')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # override the default output from pydantic by calling `to_dict()` of monitoring_window
        if self.monitoring_window:
            _dict['monitoringWindow'] = self.monitoring_window.to_dict()
        # override the default output from pydantic by calling `to_dict()` of num_moves
        if self.num_moves:
            _dict['numMoves'] = self.num_moves.to_dict()
        # override the default output from pydantic by calling `to_dict()` of hold_down_time
        if self.hold_down_time:
            _dict['holdDownTime'] = self.hold_down_time.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "id": obj.get("id"),
            "type": obj.get("type") if obj.get("type") is not None else 'NetworkInstanceBridgeTableMacDuplication',
            ...
            "adminState": NetworkInstanceBridgeTableMacDuplicationAdminState.from_dict(obj["adminState"]) if obj.get("adminState") is not None else None,
            "monitoringWindow": MonitoringWindow.from_dict(obj["monitoringWindow"]) if obj.get("monitoringWindow") is not None else None,
            "numMoves": NumMoves.from_dict(obj["numMoves"]) if obj.get("numMoves") is not None else None,
            "holdDownTime": HoldDownTime.from_dict(obj["holdDownTime"]) if obj.get("holdDownTime") is not None else None,
            "action": NetworkInstanceBridgeTableMacDuplicationAction.from_dict(obj["action"]) if obj.get("action") is not None else None,
