...
class InterfaceEthernetSymbolMonitorCurrentAlarms(BaseModel):
    """
    ...
    def value_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['none', 'sd-threshold-exceeded', 'sf-threshold-exceeded']):
            raise ValueError("must be one of enum values ('none', 'sd-threshold-exceeded', 'sf-threshold-exceeded')")
        return value
