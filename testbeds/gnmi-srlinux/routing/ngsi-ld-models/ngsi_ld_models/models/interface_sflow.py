...
class InterfaceSflow(BaseModel):
    """
    ...
    type: StrictStr = Field(description="NGSI-LD Entity identifier. It has to be InterfaceSflow.")
    ...
    admin_state: Optional[InterfaceSflowAdminState] = Field(default=None, alias="adminState")
    ingress_sampling_rate: Optional[IngressSamplingRate] = Field(default=None, alias="ingressSamplingRate")
    egress_sampling_rate: Optional[EgressSamplingRate] = Field(default=None, alias="egressSamplingRate")
    is_part_of: IsPartOf = Field(alias="isPartOf")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["id", "type", "scope", "location", "observationSpace", "operationSpace", "createdAt", "modifiedAt", "deletedAt", "adminState", "ingressSamplingRate", "egressSamplingRate", "isPartOf"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['InterfaceSflow']):
            raise ValueError("must be one of enum values ('InterfaceSflow')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # override the default output from pydantic by calling `to_dict()` of ingress_sampling_rate
        if self.ingress_sampling_rate:
            _dict['ingressSamplingRate'] = self.ingress_sampling_rate.to_dict()
        # override the default output from pydantic by calling `to_dict()` of egress_sampling_rate
        if self.egress_sampling_rate:
            _dict['egressSamplingRate'] = self.egress_sampling_rate.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "id": obj.get("id"),
            "type": obj.get("type") if obj.get("type") is not None else 'InterfaceSflow',
            ...
            "adminState": InterfaceSflowAdminState.from_dict(obj["adminState"]) if obj.get("adminState") is not None else None,
            "ingressSamplingRate": IngressSamplingRate.from_dict(obj["ingressSamplingRate"]) if obj.get("ingressSamplingRate") is not None else None,
            "egressSamplingRate": EgressSamplingRate.from_dict(obj["egressSamplingRate"]) if obj.get("egressSamplingRate") is not None else None,
