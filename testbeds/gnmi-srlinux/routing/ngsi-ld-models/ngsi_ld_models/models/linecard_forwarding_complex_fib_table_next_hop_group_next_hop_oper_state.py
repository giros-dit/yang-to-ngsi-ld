...
class LinecardForwardingComplexFibTableNextHopGroupNextHopOperState(BaseModel):
    """
    ...
    def value_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['up', 'down', 'failed', 'up-unused', 'down-vni-conflict']):
            raise ValueError("must be one of enum values ('up', 'down', 'failed', 'up-unused', 'down-vni-conflict')")
        return value
