...
class NetworkInstanceIcmpStatisticsTypeName(BaseModel):
    """
    ...
    def value_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['echo-reply', 'dest-unreachable', 'redirect', 'echo', 'rtr-advertisement', 'rtr-selection', 'time-exceeded', 'param-problem', 'timestamp', 'timestamp-reply']):
            raise ValueError("must be one of enum values ('echo-reply', 'dest-unreachable', 'redirect', 'echo', 'rtr-advertisement', 'rtr-selection', 'time-exceeded', 'param-problem', 'timestamp', 'timestamp-reply')")
        return value
