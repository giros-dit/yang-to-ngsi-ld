...
class NetworkInstanceSystemIpv4AddressOperDownReason(BaseModel):
    """
    ...
    def value_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['system-interface-not-bound', 'system-interface-has-no-ipv4-address']):
            raise ValueError("must be one of enum values ('system-interface-not-bound', 'system-interface-has-no-ipv4-address')")
        return value
