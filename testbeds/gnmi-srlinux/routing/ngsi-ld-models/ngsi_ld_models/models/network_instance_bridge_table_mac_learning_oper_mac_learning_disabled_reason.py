...
class NetworkInstanceBridgeTableMacLearningOperMacLearningDisabledReason(BaseModel):
    """
    ...
    def value_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['admin-disabled']):
            raise ValueError("must be one of enum values ('admin-disabled')")
        return value
