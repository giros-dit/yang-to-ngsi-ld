...
class NetworkInstanceConnectionPointOperDownReason(BaseModel):
    """
    ...
    def value_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['mac-dup-detected', 'associations-oper-down', 'no-associations']):
            raise ValueError("must be one of enum values ('mac-dup-detected', 'associations-oper-down', 'no-associations')")
        return value
