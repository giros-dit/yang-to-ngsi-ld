...
class NetworkInstanceProtocolsOspfInstanceAreaInterfaceTraceOptionsTracePacketModifier(BaseModel):
    """
    ...
    def value_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['ingress', 'egress', 'in-and-egress', 'drop']):
            raise ValueError("must be one of enum values ('ingress', 'egress', 'in-and-egress', 'drop')")
        return value
