...
class NetworkInstanceInterfaceBridgeTableOperMacLearningDisabledReason(BaseModel):
    """
    ...
    def value_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['admin-disabled', 'mac-dup-detected']):
            raise ValueError("must be one of enum values ('admin-disabled', 'mac-dup-detected')")
        return value
