...
class ForwardingMode(BaseModel):
    """
    ...
    def value_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['store-and-forward', 'cut-through']):
            raise ValueError("must be one of enum values ('store-and-forward', 'cut-through')")
        return value
