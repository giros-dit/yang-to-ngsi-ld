...
class SystemProtectionPoliciesPolicySeamlessBfd(BaseModel):
    """
    ...
    type: StrictStr = Field(description="NGSI-LD Entity identifier. It has to be SystemProtectionPoliciesPolicySeamlessBfd.")
    ...
    detection_multiplier: Optional[DetectionMultiplier] = Field(default=None, alias="detectionMultiplier")
    desired_minimum_transmit_interval: Optional[DesiredMinimumTransmitInterval] = Field(default=None, alias="desiredMinimumTransmitInterval")
    hold_down_timer: Optional[HoldDownTimer] = Field(default=None, alias="holdDownTimer")
    wait_for_up_timer: Optional[WaitForUpTimer] = Field(default=None, alias="waitForUpTimer")
    mode: Optional[SystemProtectionPoliciesPolicySeamlessBfdMode] = None
    threshold: Optional[Threshold] = None
    is_part_of: IsPartOf = Field(alias="isPartOf")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["id", "type", "scope", "location", "observationSpace", "operationSpace", "createdAt", "modifiedAt", "deletedAt", "detectionMultiplier", "desiredMinimumTransmitInterval", "holdDownTimer", "waitForUpTimer", "mode", "threshold", "isPartOf"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['SystemProtectionPoliciesPolicySeamlessBfd']):
            raise ValueError("must be one of enum values ('SystemProtectionPoliciesPolicySeamlessBfd')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # override the default output from pydantic by calling `to_dict()` of detection_multiplier
        if self.detection_multiplier:
            _dict['detectionMultiplier'] = self.detection_multiplier.to_dict()
        # override the default output from pydantic by calling `to_dict()` of desired_minimum_transmit_interval
        if self.desired_minimum_transmit_interval:
            _dict['desiredMinimumTransmitInterval'] = self.desired_minimum_transmit_interval.to_dict()
        # override the default output from pydantic by calling `to_dict()` of hold_down_timer
        if self.hold_down_timer:
            _dict['holdDownTimer'] = self.hold_down_timer.to_dict()
        # override the default output from pydantic by calling `to_dict()` of wait_for_up_timer
        if self.wait_for_up_timer:
            _dict['waitForUpTimer'] = self.wait_for_up_timer.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "id": obj.get("id"),
            "type": obj.get("type") if obj.get("type") is not None else 'SystemProtectionPoliciesPolicySeamlessBfd',
            ...
            "detectionMultiplier": DetectionMultiplier.from_dict(obj["detectionMultiplier"]) if obj.get("detectionMultiplier") is not None else None,
            "desiredMinimumTransmitInterval": DesiredMinimumTransmitInterval.from_dict(obj["desiredMinimumTransmitInterval"]) if obj.get("desiredMinimumTransmitInterval") is not None else None,
            "holdDownTimer": HoldDownTimer.from_dict(obj["holdDownTimer"]) if obj.get("holdDownTimer") is not None else None,
            "waitForUpTimer": WaitForUpTimer.from_dict(obj["waitForUpTimer"]) if obj.get("waitForUpTimer") is not None else None,
            "mode": SystemProtectionPoliciesPolicySeamlessBfdMode.from_dict(obj["mode"]) if obj.get("mode") is not None else None,
