...
class NetworkInstanceUdpListeningApplication(BaseModel):
    """
    ...
    type: StrictStr = Field(description="NGSI-LD Entity identifier. It has to be NetworkInstanceUdpListeningApplication.")
    ...
    local_address: Optional[NetworkInstanceUdpListeningApplicationLocalAddress] = Field(default=None, alias="localAddress")
    local_port: Optional[NetworkInstanceUdpListeningApplicationLocalPort] = Field(default=None, alias="localPort")
    process_id: Optional[NetworkInstanceUdpListeningApplicationProcessId] = Field(default=None, alias="processId")
    is_part_of: IsPartOf = Field(alias="isPartOf")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["id", "type", "scope", "location", "observationSpace", "operationSpace", "createdAt", "modifiedAt", "deletedAt", "localAddress", "localPort", "processId", "isPartOf"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['NetworkInstanceUdpListeningApplication']):
            raise ValueError("must be one of enum values ('NetworkInstanceUdpListeningApplication')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "id": obj.get("id"),
            "type": obj.get("type") if obj.get("type") is not None else 'NetworkInstanceUdpListeningApplication',
            ...
            "localAddress": NetworkInstanceUdpListeningApplicationLocalAddress.from_dict(obj["localAddress"]) if obj.get("localAddress") is not None else None,
            "localPort": NetworkInstanceUdpListeningApplicationLocalPort.from_dict(obj["localPort"]) if obj.get("localPort") is not None else None,
            "processId": NetworkInstanceUdpListeningApplicationProcessId.from_dict(obj["processId"]) if obj.get("processId") is not None else None,
