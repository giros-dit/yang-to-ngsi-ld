...
class ControlInterface(BaseModel):
    ...
    type: StrictStr = Field(description="NGSI-LD Entity identifier. It has to be ControlInterface.")
    ...
    name: Optional[ControlInterfaceName] = None
    admin_state: Optional[ControlInterfaceAdminState] = Field(default=None, alias="adminState")
    oper_state: Optional[ControlInterfaceOperState] = Field(default=None, alias="operState")
    is_part_of: IsPartOf = Field(alias="isPartOf")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["id", "type", "scope", "location", "observationSpace", "operationSpace", "createdAt", "modifiedAt", "deletedAt", "name", "adminState", "operState", "isPartOf"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['ControlInterface']):
            raise ValueError("must be one of enum values ('ControlInterface')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "id": obj.get("id"),
            "type": obj.get("type") if obj.get("type") is not None else 'ControlInterface',
            ...
            "name": ControlInterfaceName.from_dict(obj["name"]) if obj.get("name") is not None else None,
            "adminState": ControlInterfaceAdminState.from_dict(obj["adminState"]) if obj.get("adminState") is not None else None,
            "operState": ControlInterfaceOperState.from_dict(obj["operState"]) if obj.get("operState") is not None else None,
