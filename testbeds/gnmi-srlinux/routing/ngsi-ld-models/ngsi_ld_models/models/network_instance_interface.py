...
class NetworkInstanceInterface(BaseModel):
    """
    ...
    type: StrictStr = Field(description="NGSI-LD Entity identifier. It has to be NetworkInstanceInterface.")
    ...
    name: Optional[NetworkInstanceInterfaceName] = None
    oper_state: Optional[NetworkInstanceInterfaceOperState] = Field(default=None, alias="operState")
    oper_down_reason: Optional[NetworkInstanceInterfaceOperDownReason] = Field(default=None, alias="operDownReason")
    index: Optional[NetworkInstanceInterfaceIndex] = None
    connection_point: Optional[NetworkInstanceInterfaceConnectionPoint] = Field(default=None, alias="connectionPoint")
    is_part_of: IsPartOf = Field(alias="isPartOf")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["id", "type", "scope", "location", "observationSpace", "operationSpace", "createdAt", "modifiedAt", "deletedAt", "name", "operState", "operDownReason", "index", "connectionPoint", "isPartOf"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['NetworkInstanceInterface']):
            raise ValueError("must be one of enum values ('NetworkInstanceInterface')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "id": obj.get("id"),
            "type": obj.get("type") if obj.get("type") is not None else 'NetworkInstanceInterface',
            ...
            "name": NetworkInstanceInterfaceName.from_dict(obj["name"]) if obj.get("name") is not None else None,
            "operState": NetworkInstanceInterfaceOperState.from_dict(obj["operState"]) if obj.get("operState") is not None else None,
            "operDownReason": NetworkInstanceInterfaceOperDownReason.from_dict(obj["operDownReason"]) if obj.get("operDownReason") is not None else None,
            "index": NetworkInstanceInterfaceIndex.from_dict(obj["index"]) if obj.get("index") is not None else None,
            "connectionPoint": NetworkInstanceInterfaceConnectionPoint.from_dict(obj["connectionPoint"]) if obj.get("connectionPoint") is not None else None,
