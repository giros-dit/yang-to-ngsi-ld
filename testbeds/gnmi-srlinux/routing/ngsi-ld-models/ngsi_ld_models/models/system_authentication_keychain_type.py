...
class SystemAuthenticationKeychainType(BaseModel):
    """
    ...
    def value_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['tcp-md5', 'isis', 'ospf', 'tcp-ao', 'vrrp', 'macsec', 'macsec-fallback']):
            raise ValueError("must be one of enum values ('tcp-md5', 'isis', 'ospf', 'tcp-ao', 'vrrp', 'macsec', 'macsec-fallback')")
        return value
