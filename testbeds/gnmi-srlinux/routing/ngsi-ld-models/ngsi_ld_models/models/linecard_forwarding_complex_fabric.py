...
class LinecardForwardingComplexFabric(BaseModel):
    """
    ...
    type: StrictStr = Field(description="NGSI-LD Entity identifier. It has to be LinecardForwardingComplexFabric.")
    ...
    total_capacity: Optional[TotalCapacity] = Field(default=None, alias="totalCapacity")
    operational_capacity: Optional[OperationalCapacity] = Field(default=None, alias="operationalCapacity")
    consumed_capacity: Optional[ConsumedCapacity] = Field(default=None, alias="consumedCapacity")
    utilization_ingress: Optional[UtilizationIngress] = Field(default=None, alias="utilizationIngress")
    utilization_egress: Optional[UtilizationEgress] = Field(default=None, alias="utilizationEgress")
    is_part_of: IsPartOf = Field(alias="isPartOf")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["id", "type", "scope", "location", "observationSpace", "operationSpace", "createdAt", "modifiedAt", "deletedAt", "availability", "totalCapacity", "operationalCapacity", "consumedCapacity", "utilizationIngress", "utilizationEgress", "isPartOf"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['LinecardForwardingComplexFabric']):
            raise ValueError("must be one of enum values ('LinecardForwardingComplexFabric')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # override the default output from pydantic by calling `to_dict()` of total_capacity
        if self.total_capacity:
            _dict['totalCapacity'] = self.total_capacity.to_dict()
        # override the default output from pydantic by calling `to_dict()` of operational_capacity
        if self.operational_capacity:
            _dict['operationalCapacity'] = self.operational_capacity.to_dict()
        # override the default output from pydantic by calling `to_dict()` of consumed_capacity
        if self.consumed_capacity:
            _dict['consumedCapacity'] = self.consumed_capacity.to_dict()
        # override the default output from pydantic by calling `to_dict()` of utilization_ingress
        if self.utilization_ingress:
            _dict['utilizationIngress'] = self.utilization_ingress.to_dict()
        # override the default output from pydantic by calling `to_dict()` of utilization_egress
        if self.utilization_egress:
            _dict['utilizationEgress'] = self.utilization_egress.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "id": obj.get("id"),
            "type": obj.get("type") if obj.get("type") is not None else 'LinecardForwardingComplexFabric',
            ...
            "totalCapacity": TotalCapacity.from_dict(obj["totalCapacity"]) if obj.get("totalCapacity") is not None else None,
            "operationalCapacity": OperationalCapacity.from_dict(obj["operationalCapacity"]) if obj.get("operationalCapacity") is not None else None,
            "consumedCapacity": ConsumedCapacity.from_dict(obj["consumedCapacity"]) if obj.get("consumedCapacity") is not None else None,
            "utilizationIngress": UtilizationIngress.from_dict(obj["utilizationIngress"]) if obj.get("utilizationIngress") is not None else None,
            "utilizationEgress": UtilizationEgress.from_dict(obj["utilizationEgress"]) if obj.get("utilizationEgress") is not None else None,
