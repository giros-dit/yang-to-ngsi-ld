...
class LastOverloadExitCode(BaseModel):
    """
    ...
    def value_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['none', 'bgp-sig-recv', 'timer-expired', 'manual-exit', 'sfm-overload-done']):
            raise ValueError("must be one of enum values ('none', 'bgp-sig-recv', 'timer-expired', 'manual-exit', 'sfm-overload-done')")
        return value
