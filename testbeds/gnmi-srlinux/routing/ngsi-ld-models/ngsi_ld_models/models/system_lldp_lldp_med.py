...
class SystemLldpLldpMed(BaseModel):
    """
     YANG module: srl_nokia-lldp.yang 
    """ # noqa: E501
    id: Optional[StrictStr] = Field(default=None, description="Entity id. ")
    type: StrictStr = Field(description="NGSI-LD Entity identifier. It has to be SystemLldpLldpMed.")
    ...
    med_fast_transmit_interval: Optional[MedFastTransmitInterval] = Field(default=None, alias="medFastTransmitInterval")
    med_fast_transmit_repeat_count: Optional[MedFastTransmitRepeatCount] = Field(default=None, alias="medFastTransmitRepeatCount")
    topology_change_notification: Optional[TopologyChangeNotification] = Field(default=None, alias="topologyChangeNotification")
    is_part_of: IsPartOf = Field(alias="isPartOf")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["id", "type", "scope", "location", "observationSpace", "operationSpace", "createdAt", "modifiedAt", "deletedAt", "medFastTransmitInterval", "medFastTransmitRepeatCount", "topologyChangeNotification", "isPartOf"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['SystemLldpLldpMed']):
            raise ValueError("must be one of enum values ('SystemLldpLldpMed')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # override the default output from pydantic by calling `to_dict()` of med_fast_transmit_interval
        if self.med_fast_transmit_interval:
            _dict['medFastTransmitInterval'] = self.med_fast_transmit_interval.to_dict()
        # override the default output from pydantic by calling `to_dict()` of med_fast_transmit_repeat_count
        if self.med_fast_transmit_repeat_count:
            _dict['medFastTransmitRepeatCount'] = self.med_fast_transmit_repeat_count.to_dict()
        # override the default output from pydantic by calling `to_dict()` of topology_change_notification
        if self.topology_change_notification:
            _dict['topologyChangeNotification'] = self.topology_change_notification.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "id": obj.get("id"),
            "type": obj.get("type") if obj.get("type") is not None else 'SystemLldpLldpMed',
            ...
            "medFastTransmitInterval": MedFastTransmitInterval.from_dict(obj["medFastTransmitInterval"]) if obj.get("medFastTransmitInterval") is not None else None,
            "medFastTransmitRepeatCount": MedFastTransmitRepeatCount.from_dict(obj["medFastTransmitRepeatCount"]) if obj.get("medFastTransmitRepeatCount") is not None else None,
            "topologyChangeNotification": TopologyChangeNotification.from_dict(obj["topologyChangeNotification"]) if obj.get("topologyChangeNotification") is not None else None,
