...
class SystemLldpLldpMedNetworkPolicy(BaseModel):
    """
    ...
    type: StrictStr = Field(description="NGSI-LD Entity identifier. It has to be SystemLldpLldpMedNetworkPolicy.")
    ...
    name: Optional[SystemLldpLldpMedNetworkPolicyName] = None
    application_type: Optional[ApplicationType] = Field(default=None, alias="applicationType")
    vlan_id: Optional[SystemLldpLldpMedNetworkPolicyVlanId] = Field(default=None, alias="vlanId")
    dot1p: Optional[SystemLldpLldpMedNetworkPolicyDot1p] = None
    dscp: Optional[SystemLldpLldpMedNetworkPolicyDscp] = None
    is_part_of: IsPartOf = Field(alias="isPartOf")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["id", "type", "scope", "location", "observationSpace", "operationSpace", "createdAt", "modifiedAt", "deletedAt", "name", "applicationType", "vlanId", "dot1p", "dscp", "isPartOf"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['SystemLldpLldpMedNetworkPolicy']):
            raise ValueError("must be one of enum values ('SystemLldpLldpMedNetworkPolicy')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # override the default output from pydantic by calling `to_dict()` of application_type
        if self.application_type:
            _dict['applicationType'] = self.application_type.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "id": obj.get("id"),
            "type": obj.get("type") if obj.get("type") is not None else 'SystemLldpLldpMedNetworkPolicy',
            ...
            "name": SystemLldpLldpMedNetworkPolicyName.from_dict(obj["name"]) if obj.get("name") is not None else None,
            "applicationType": ApplicationType.from_dict(obj["applicationType"]) if obj.get("applicationType") is not None else None,
            "vlanId": SystemLldpLldpMedNetworkPolicyVlanId.from_dict(obj["vlanId"]) if obj.get("vlanId") is not None else None,
            "dot1p": SystemLldpLldpMedNetworkPolicyDot1p.from_dict(obj["dot1p"]) if obj.get("dot1p") is not None else None,
            "dscp": SystemLldpLldpMedNetworkPolicyDscp.from_dict(obj["dscp"]) if obj.get("dscp") is not None else None,
