# InterfaceSubinterfaceIpv4StatisticsInTerminatedPackets

The total number of input packets that were received on this subinterface that were extracted to the control plane  The count includes packets eventually discarded by the CPM. Such discards include: - packets with unsupported IP protocol numbers - packets destined to TCP/UDP ports that are not open/listening - IPv4 packets with any IP options - IPv6 packets with any extension headers  YANG module: srl_nokia-interfaces.yang 
...
interface_subinterface_ipv4_statistics_in_terminated_packets_from_dict = InterfaceSubinterfaceIpv4StatisticsInTerminatedPackets.from_dict(interface_subinterface_ipv4_statistics_in_terminated_packets_dict)
```
