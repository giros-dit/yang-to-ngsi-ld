...
performance_measurements_file = open("/opt/network-controller-virtualization/network_controller_virtualization/performance_measurements_operation.csv", "w", newline='')
...
def get_xpath_in_context_catalog(entity_type: str, all_context_data: Optional[dict]) -> str:
    ...
    for key, value in all_context_data.items():
        if isinstance(value, list):
            for v in value:
                if v == entity_type:
                    xpath =  "/" + entity_type#.split(":", 1)[1]
                    entity_type_short = key
                    logger.info(f"Entity type name {entity_type_short} relative to NGSI-LD Entity type URI {entity_type} was founded!")
                    logger.info("The XPath relative to the NGSI-LD Entity type " + entity_type + " is " + xpath)
        ...
        logger.info(f"Entity type name relative to NGSI-LD Entity type URI {entity_type} was not founded!")
...
'''
Discover if the NGSI-LD Entity is configurable or not by providing the entity type in the short form (i.e., the entity type name).
'''
def discover_config_entity_by_name(entity_type: str, all_context_registries: Optional[list]) -> bool:
            ...
            if short_entity_type != None and "config" in context_registry.keys():
                ...
                for config_element in config_elements:
                    if short_entity_type == config_element:
                        configurable = True
                        logger.info("Entity type " + short_entity_type + " is configurable!")
                        break
...
'''
def get_operation(host: str, port: str, username: str, password: str,  entity_type: str, entity_id: str, option: str, notified_at: str, all_context_data: Optional[dict] = None, sysAttrs: Optional[bool] = False, all_context_registries: Optional[list] = None):

    # Load the YAML configuration file
    yaml_file = '/opt/network-controller-virtualization/network_controller_virtualization/gnmic-templates/gnmic_query_template.yaml'
    output_file = '/opt/network-controller-virtualization/network_controller_virtualization/gnmic-templates/gnmic_query_template_updated.yaml'
    ...
    query = subprocess.run(["gnmic", "get", "--config",  "/opt/network-controller-virtualization/network_controller_virtualization/gnmic-templates/gnmic_query_template_updated.yaml"], capture_output=True, text=True)
    ...
    query_output[0]["sysAttrs"] = sysAttrs
