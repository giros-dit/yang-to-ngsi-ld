...
'''
def get_operation(host: str, port: str, username: str, password: str,  entity_type: str, entity_id: str, option: str, all_context_data: Optional[dict] = None, sysAttrs: Optional[bool] = False, all_context_registries: Optional[list] = None):
    ...
    logger.info("CHACHOOO AMOS A VER")
    if entity_id != None:
        logger.info("CHACHOOO AMOS A VER 1")
        xpath = get_xpath_with_keys(xpath=xpath, entity_id=entity_id, all_context_registries=all_context_registries)
    else: 
        logger.info("CHACHOOO AMOS A VER 2")
    
    xpath = "/" + str(str(xpath).split("/")[1])

    logger.info("CHACHO the XPath is " + str(xpath))
