...
class VlanState(BaseModel):
    ...
    type: StrictStr = Field(description="NGSI-LD Entity identifier. It has to be VlanState.")
    ...
    vlan_id: Optional[VlanStateVlanId] = Field(default=None, alias="vlanId")
    name: Optional[VlanStateName] = None
    status: Optional[VlanStateStatus] = None
    ...
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('VlanState'):
            raise ValueError("must be one of enum values ('VlanState')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "id": obj.get("id"),
            "type": obj.get("type") if obj.get("type") is not None else 'VlanState',
            ...
            "vlanId": VlanStateVlanId.from_dict(obj["vlanId"]) if obj.get("vlanId") is not None else None,
            "name": VlanStateName.from_dict(obj["name"]) if obj.get("name") is not None else None,
            "status": VlanStateStatus.from_dict(obj["status"]) if obj.get("status") is not None else None,
