...
def parse_gnmi_notification(message):
    dict_buffers = []
    data = message.value
    ...
    for item in data:
        ...
        for key, value in item['values'].items():
            ...
            values.append(value)
        source = "-".join(item['tags']['source'].split("-")[0:-1]) + ":" + str(item['tags']['source'].split("-")[-1])
        timestamp_data = int(item['timestamp'])
        sysAttrs = str(item['sysAttrs'])
        ...
        #observed_at = str(datetime_ns.astype('datetime64[ns]')) + 'Z'
