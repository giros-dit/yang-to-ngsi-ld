...
class InterfaceAggregationState(BaseModel):
    """
    ...
    type: StrictStr = Field(description="NGSI-LD Entity identifier. It has to be InterfaceAggregationState.")
    ...
    lag_type: Optional[InterfaceAggregationStateLagType] = Field(default=None, alias="lagType")
    min_links: Optional[InterfaceAggregationStateMinLinks] = Field(default=None, alias="minLinks")
    lag_speed: Optional[LagSpeed] = Field(default=None, alias="lagSpeed")
    member: Optional[InterfaceAggregationStateMember] = None
    is_part_of: IsPartOf = Field(alias="isPartOf")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["id", "type", "scope", "location", "observationSpace", "operationSpace", "createdAt", "modifiedAt", "deletedAt", "lagType", "minLinks", "lagSpeed", "member", "isPartOf"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('InterfaceAggregationState'):
            raise ValueError("must be one of enum values ('InterfaceAggregationState')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # override the default output from pydantic by calling `to_dict()` of lag_type
        if self.lag_type:
            _dict['lagType'] = self.lag_type.to_dict()
        # override the default output from pydantic by calling `to_dict()` of min_links
        if self.min_links:
            _dict['minLinks'] = self.min_links.to_dict()
        # override the default output from pydantic by calling `to_dict()` of lag_speed
        if self.lag_speed:
            _dict['lagSpeed'] = self.lag_speed.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "id": obj.get("id"),
            "type": obj.get("type") if obj.get("type") is not None else 'InterfaceAggregationState',
            ...
            "lagType": InterfaceAggregationStateLagType.from_dict(obj["lagType"]) if obj.get("lagType") is not None else None,
            "minLinks": InterfaceAggregationStateMinLinks.from_dict(obj["minLinks"]) if obj.get("minLinks") is not None else None,
            "lagSpeed": LagSpeed.from_dict(obj["lagSpeed"]) if obj.get("lagSpeed") is not None else None,
            "member": InterfaceAggregationStateMember.from_dict(obj["member"]) if obj.get("member") is not None else None,
