...
class VlanConfig(BaseModel):
    ...
    type: StrictStr = Field(description="NGSI-LD Entity identifier. It has to be VlanConfig.")
    ...
    vlan_id: Optional[VlanConfigVlanId] = Field(default=None, alias="vlanId")
    name: Optional[VlanConfigName] = None
    status: Optional[VlanConfigStatus] = None
    is_part_of: IsPartOf = Field(alias="isPartOf")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["id", "type", "scope", "location", "observationSpace", "operationSpace", "createdAt", "modifiedAt", "deletedAt", "vlanId", "name", "status", "isPartOf"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('VlanConfig'):
            raise ValueError("must be one of enum values ('VlanConfig')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "id": obj.get("id"),
            "type": obj.get("type") if obj.get("type") is not None else 'VlanConfig',
            ...
            "vlanId": VlanConfigVlanId.from_dict(obj["vlanId"]) if obj.get("vlanId") is not None else None,
            "name": VlanConfigName.from_dict(obj["name"]) if obj.get("name") is not None else None,
            "status": VlanConfigStatus.from_dict(obj["status"]) if obj.get("status") is not None else None,
