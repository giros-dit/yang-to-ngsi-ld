...
class QuerySubscription200ResponseInner(BaseModel):
    """
    ...
    entities: Optional[Annotated[List[EntitySelector], Field(min_length=1)]] = Field(default=None, description="Entities subscribed.  Mandatory if timeInterval is present, unless the execution of the request  is limited to local scope (see clause 5.5.13). ")
    local_only: Optional[StrictBool] = Field(default=None, description="If localOnly=true then the subscription only pertains to the Entities  stored locally (see clause 5.5.13). ", alias="localOnly")
    ...
    __properties: ClassVar[List[str]] = ["id", "type", "subscriptionName", "description", "entities", "localOnly", "notificationTrigger", "q", "geoQ", "csf", "isActive", "notification", "expiresAt", "temporalQ", "scopeQ", "lang", "createdAt", "modifiedAt", "deletedAt", "status", "jsonldContext", "datasetId", "watchedAttributes", "throttling", "timeInterval"]
    ...
    def notification_trigger_validate_enum(cls, value):
        ...
        for i in value:
            if i not in set(['entityCreated', 'entityUpdated', 'entityDeleted', 'attributeCreated', 'attributeUpdated', 'attributeDeleted']):
    ...
    def status_validate_enum(cls, value):
        ...
        if value not in set(['active', 'paused', 'expired']):
